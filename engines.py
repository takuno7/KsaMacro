...
class KsaEngine:
    ...
    def search_departures(
        ...
        data = res_json.get("data", {})
        schedule_list = data.get("result", [])
        all_rooms = data.get("resultAll", [])
    ...
    def book(
        ...
        Args:
            ...
            card_info: 카드 결제 정보 (cardno, validdate, password, requestno). 없으면 좌석 선점까지만 진행.
        ...
        try:
            ...
            td_list = dc_resp.json().get("data", {}).get("tdResult", [])
            if td_list:
                for td in td_list:
                    if str(td.get("ticketid")) == "1" and td.get("dcid"):
                        default_dcid = str(td.get("dcid"))
                        break
        except Exception:
            pass
        ...
        for i, p in enumerate(formatted_passengers):
            ...
            params_list.append({
                ...
                "cshipping": "0",
                "cdischarge": "0",
                "memberid": self.user_id if self.is_logged_in else "Default",
        ...
        chk_res_list = chk_p_json.get("data", {}).get("result", [])
        if not chk_res_list or chk_res_list[0].get("errcode") != 0:
        ...
        cap_resp = self.session.post(url_cap, data={"TicketingKey": ticketingkey}, timeout=10)
        cap_json = cap_resp.json()
        _log("INFO", f"좌석 배정 응답: result={cap_json.get('result')}, errCode={cap_json.get('errCode')}")

        if not cap_json.get("result", False) or cap_json.get("errCode", -1) != 0:
            err_msg = cap_json.get("message") or "좌석 할당 실패"
            raise KsaError(err_msg)

        seats = cap_json.get("data", {}).get("result", [])
        ...
        # 카드 결제 정보가 없으면 좌석 선점 완료 상태로 반환
        if not card_info or not card_info.get("cardno"):
            _log("INFO", "카드 결제 정보가 없어 좌석 선점 상태로 종료합니다.")
            return {
                "success": True,
        ...
        app_resp = self.session.post(url_approve, data=approve_params, timeout=15)
        ...
        if not app_json.get("result", False) or app_data.get("errCode", -1) != 0:
            ...
            try:
                self.session.post(f"{BASE_URL}/booking/ticketRollBack", data={
                    ...
                    "groupID": "0",
                    "approvekind": "presale",
                    "messageRB": f"Main[[F1]{err_msg}]"
                }, timeout=5)
            except Exception:
                pass
            raise KsaError(f"카드 결제 승인 실패: {err_msg}")
        ...
        comp_resp = self.session.post(url_complete, data={
            "jsonstring": json.dumps(params_list, ensure_ascii=False),
            "fcmtoken": ""
        }, timeout=15)
    ...
    def get_reservations(self, group_id: str = "") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"예매 내역 조회 실패: {e}")
        return []
